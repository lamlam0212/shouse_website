...
export default async function AdminProducts({ searchParams }: { searchParams: Promise<Params> }) {
  ...
  return <div className="admin-page">
    <div className="admin-page-head"><div><span>Quản lý nội dung</span><h1>Sản phẩm</h1><p>{result.total} sản phẩm phù hợp với bộ lọc hiện tại.</p></div><Link href="/admin/san-pham/moi" className="button button-primary"><Plus/> Thêm sản phẩm</Link></div>
    ...
    <div className="admin-card">
      ...
      <div className="admin-table-wrap"><table className="admin-table"><thead><tr><th>Tên sản phẩm</th><th>Danh mục</th><th>Giá</th><th>Trạng thái</th><th></th></tr></thead><tbody>{result.items.map(product=><tr key={product.id}><td><strong>{product.name}</strong><small>{product.code}</small></td><td>{product.categoryName}</td><td>{formatPrice(product.price)}</td><td><span className={`status ${product.status}`}>{product.status==="published"?"Đã xuất bản":product.status==="draft"?"Bản nháp":"Đã ẩn"}</span></td><td><ProductActions id={product.id} slug={product.slug} status={product.status}/></td></tr>)}{!result.items.length&&<tr><td colSpan={5}>Không tìm thấy sản phẩm phù hợp.</td></tr>}</tbody></table></div>
    </div>
