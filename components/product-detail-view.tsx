import Link from "next/link";
import { AlignLeft, ArrowLeft, Download, FileText, ListChecks, MessageCircle, Phone } from "lucide-react";
import type { Product, SiteSettings } from "@/lib/models";
import { formatPrice } from "@/lib/models";
import { ProductGallery } from "./product-gallery";
import { RichDescription } from "./rich-description";

export function ProductDetailView({product,settings,preview=false}:{product:Product;settings:SiteSettings;preview?:boolean}){
  const phone=settings.hotline.replace(/\s/g,"");const zalo=settings.zalo.replace(/\s/g,"");
  return <>
    {preview&&<div className="preview-banner"><strong>Chế độ xem trước quản trị</strong><span>Chỉ hiển thị dữ liệu đã lưu gần nhất.</span></div>}
    <section className="product-detail"><div className="container">
      <Link href={preview?`/admin/san-pham/${product.id}`:"/san-pham"} className="back-link"><ArrowLeft size={17}/> {preview?"Quay lại chỉnh sửa":"Quay lại danh sách"}</Link>
      <div className="detail-grid"><ProductGallery product={product}/><div className="detail-info">
        <span className="eyebrow">{product.categoryName}</span><h1>{product.name}</h1>
        <p className="product-code">Mã sản phẩm: <strong>{product.code}</strong></p>{product.shortDescription&&<p className="lead">{product.shortDescription}</p>}
        <div className="detail-price"><span>Giá</span><strong>{formatPrice(product.price)}</strong></div>
        {(phone||zalo)&&<div className="detail-actions">{phone&&<a className="button button-primary" href={`tel:${phone}`}><Phone size={18}/> Gọi tư vấn</a>}{zalo&&<a className="button button-secondary" href={`https://zalo.me/${zalo}`} target="_blank" rel="noreferrer"><MessageCircle size={18}/> Nhắn Zalo</a>}</div>}
      </div></div>
    </div></section>
    <section className="section section-soft"><div className="container detail-content">
      <article className="detail-section-card description-card"><header className="detail-section-heading"><i><AlignLeft/></i><div><span className="section-kicker">Thông tin sản phẩm</span><h2>Mô tả chi tiết</h2></div></header><RichDescription content={product.description}/>{product.pdfPath&&<div className="pdf-card"><FileText/><div><strong>Tài liệu sản phẩm</strong><span>Định dạng PDF</span></div>{product.pdfUrl?<a href={product.pdfUrl} target="_blank" rel="noreferrer"><Download/> Tải PDF</a>:<span>Không thể tải file</span>}</div>}</article>
      <aside className="detail-section-card specs-card"><header className="detail-section-heading"><i><ListChecks/></i><div><span className="section-kicker">Thông số</span><h2>Bảng thông số kỹ thuật</h2></div></header>{product.specs.length?<div className="spec-table">{product.specs.map(spec=><div key={spec.id||spec.name}><span>{spec.name}</span><strong>{spec.value}{spec.unit&&` ${spec.unit}`}</strong></div>)}</div>:<p className="empty-copy">Thông số đang được cập nhật.</p>}</aside>
    </div></section>
  </>;
}
